									...
									try {
										...
										if (event.getGuild() instanceof final Guild guild) {
											if (guild.getTextChannelById(config.getLogChannelId()) instanceof final TextChannel channel) {
												channel.sendMessageEmbeds(ErrorEmbed.of(event.getInteraction(), e)).queue();
											}
										}
										interactionHook.editOriginal(R.Strings.ui("an_error_occurred_the_developer_has_been_notified"))
												.queue();
										//#if dev
