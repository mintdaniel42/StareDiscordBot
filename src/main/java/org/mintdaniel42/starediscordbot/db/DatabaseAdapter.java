...
public final class DatabaseAdapter implements AutoCloseable {
		...
		try {
			...
			while (metaDataModelDao.queryForFirst().getVersion() != MetaDataModel.Version.values()[MetaDataModel.Version.values().length - 1]) {
				switch (metaDataModelDao.queryForFirst().getVersion()) {
						...
						try {
							...
							List<UUID> uuids = Stream.of(hnsUserModelDao.queryForAll(), pgUserModelDao.queryForAll())
									...
									.map(object -> {
										...
										else if (object instanceof PGUserModel pgUserModel)
											return pgUserModel.getUuid();
										else return null;
									})
					...
					case HNS_V2 -> {
						try {
							for (UUID uuid : groupModelDao.queryForAll()
									.stream()
									.map(GroupModel::getLeader)
									.toList()) {
								userModelDao.update(userModelDao.queryForId(uuid)
										.toBuilder()
										.group(groupModelDao.queryBuilder()
												.where()
												.eq("leader", uuid)
												.queryForFirst()).build());
							}

							metaDataModelDao.update(new MetaDataModel(MetaDataModel.Version.LEADER_BUG));
						} catch (SQLException e) {
							log.error(R.Strings.log("could_not_perform_s_migration", "leader_bug"), e);
							throw new RuntimeException(e);
						}
					}
				}
	...
	public boolean addGroup(@NonNull GroupModel groupModel) {
		try {
			userModelDao.update(userModelDao.queryForId(groupModel.getLeader())
					.toBuilder()
					.group(groupModel)
					.build());
			...
			return switch (model) {
				...
				case GroupModel groupModel -> {
					userModelDao.update(userModelDao.queryForId(groupModel.getLeader())
							...
							.group(groupModel)
							.build());
					yield groupModelDao.update(groupModel) != 0;
				}
